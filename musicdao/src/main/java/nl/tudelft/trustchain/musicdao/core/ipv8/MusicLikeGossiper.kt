...
class MusicLikeGossiper
    ...
    constructor(
        ...
        fun gossip() {
            ...
            val releaseBlocks =
                musicCommunity.database
                    .getBlocksWithType(MusicLikeBlock.BLOCK_TYPE)
                    .filter {
                        Log.d("MusicLike", "Found block with blockId: ${it.type} ${it.transaction}")
                        musicLikeBlockValidator.validateTransaction(it.transaction)
                    }.shuffled()
                    .take(Config.BLOCKS)
        ...
        public fun pickRandomPeer(): Peer? {
