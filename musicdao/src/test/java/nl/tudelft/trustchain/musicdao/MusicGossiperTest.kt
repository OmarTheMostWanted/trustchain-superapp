...
class MusicGossiperTest {
    ...
    private lateinit var blockValidator: MusicLikeBlockValidator
    private lateinit var gossiper: MusicLikeGossiper
    ...
    fun setup() {
        ...
        gossiper = spyk(MusicLikeGossiper(musicCommunity, blockValidator))
    }
    ...
    fun testGossipRandomPeer() =
        runTest {
            ...
            every { musicCommunity.database.getBlocksWithType(MusicLikeBlock.BLOCK_TYPE) } returns listOf(mockBlock, mockBlock)
    ...
    fun testGossipNoPeers() =
        runTest {
            ...
            every { musicCommunity.database.getBlocksWithType(MusicLikeBlock.BLOCK_TYPE) } returns listOf(mockBlock)
            every { blockValidator.validateTransaction(any()) } returns true
